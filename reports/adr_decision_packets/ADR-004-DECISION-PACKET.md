...
Should **ADR-004** be accepted as the governing Decision Log decision for **Authentication Provider**?

If accepting the IS-101 recommendation: Accept Supabase Auth AND select auth method (OAuth vs magic-link/password)

## 3. Background

- Why open: IS-101 recommendation recorded; Decision Log acceptance never completed.
- Why it matters: ISSUE-AUTH-001 method unresolved; Catalog 4 seed conflict risk
...
- Description: Accept Supabase Auth + Google OAuth method
- G-10: Disposition in Decision Log does not alone open G-10.
- Reversibility: SUPERSEDE via new ADR if needed.

### Option 2

- Description: Accept Supabase Auth + magic-link/password method
- G-10: Disposition in Decision Log does not alone open G-10.
- Reversibility: SUPERSEDE via new ADR if needed.

### Option 3

- Description: Accept Supabase Auth with method deferred by explicit Decision Log (weak)
- G-10: Disposition in Decision Log does not alone open G-10.
- Reversibility: SUPERSEDE via new ADR if needed.

### Option 4

- Description: Reject provider recommendation
- G-10: Disposition in Decision Log does not alone open G-10.
- Reversibility: SUPERSEDE via new ADR if needed.
...
Accept Supabase Auth AND select auth method (OAuth vs magic-link/password)

This is **not** Steve's decision. Label: **PROPOSED FOR STEVE DECISION**.

## 6. Acceptance criteria (if Steve accepts)

1. Decision Log entry accepting ADR-004 with selected option.
...
- IS-101 ADR-004; `docs/adr/_index.md`
...
- G-10 effect: BLOCKING_G10 | MG effect: Blocks auth-related tables/packages
