...
Should **ADR-003** be accepted as the governing Decision Log decision for **ORM / Data Access**?

If accepting the IS-101 recommendation: Accept Prisma behind repository adapters (domain never imports Prisma Client)

## 3. Background

- Why open: IS-101 recommendation recorded; Decision Log acceptance never completed.
- Why it matters: Controls translation layer for MG-*
...
- Description: Accept Prisma behind adapters
- G-10: Disposition in Decision Log does not alone open G-10.
- Reversibility: SUPERSEDE via new ADR if needed.

### Option 2

- Description: Accept alternate ORM/query layer with new evaluation
- G-10: Disposition in Decision Log does not alone open G-10.
- Reversibility: SUPERSEDE via new ADR if needed.

### Option 3

- Description: Defer (blocks MG translation clarity)
- G-10: Disposition in Decision Log does not alone open G-10.
- Reversibility: SUPERSEDE via new ADR if needed.
...
Accept Prisma behind repository adapters (domain never imports Prisma Client)

This is **not** Steve's decision. Label: **PROPOSED FOR STEVE DECISION**.

## 6. Acceptance criteria (if Steve accepts)

1. Decision Log entry accepting ADR-003 with selected option.
...
- IS-101 ADR-003; `docs/adr/_index.md`
