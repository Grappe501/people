...
Should **ADR-009** be accepted as the governing Decision Log decision for **Hosting / Deployment**?

If accepting the IS-101 recommendation: Dedicated Netlify site (D-018) + external backends

## 3. Background

- Why open: IS-101 recommendation recorded; Decision Log acceptance never completed.
- Why it matters: D-018 already site dedicated
...
Dedicated Netlify site (D-018) + external backends

This is **not** Steve's decision. Label: **PROPOSED FOR STEVE DECISION**.

## 6. Acceptance criteria (if Steve accepts)

1. Decision Log entry accepting ADR-009 with selected option.
...
- IS-101 ADR-009; `docs/adr/_index.md`
...
- G-10 effect: CONDITIONAL | MG effect: Deploy packages
