...
Should **ADR-002** be accepted as the governing Decision Log decision for **Database Provider**?

If accepting the IS-101 recommendation: Accept hosted PostgreSQL (brand may remain TBD among Postgres-class hosts)

## 3. Background

- Why open: IS-101 recommendation recorded; Decision Log acceptance never completed.
- Why it matters: Required before physical schema / ISSUE-DATABASE-001
...
- Description: Accept hosted PostgreSQL (brand TBD allowed if Decision Log states)
- G-10: Disposition in Decision Log does not alone open G-10.
- Reversibility: SUPERSEDE via new ADR if needed.

### Option 2

- Description: Accept specific brand now (e.g. Supabase PG)
- G-10: Disposition in Decision Log does not alone open G-10.
- Reversibility: SUPERSEDE via new ADR if needed.

### Option 3

- Description: Reject / require alternate engine
- G-10: Disposition in Decision Log does not alone open G-10.
- Reversibility: SUPERSEDE via new ADR if needed.
...
Accept hosted PostgreSQL (brand may remain TBD among Postgres-class hosts)

This is **not** Steve's decision. Label: **PROPOSED FOR STEVE DECISION**.

## 6. Acceptance criteria (if Steve accepts)

1. Decision Log entry accepting ADR-002 with selected option.
...
- IS-101 ADR-002; `docs/adr/_index.md`
...
- G-10 effect: BLOCKING_G10 | MG effect: BLOCKING_MG
