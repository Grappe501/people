...
| --- | --- |
| ADR ID | ADR-008 |
| Title | API Style / Versioning |
| Current status | OPEN / PROPOSED (not accepted) |
| Decision owner | Steve |
| Prepared by | Burt (Cursor) |
| Prepared against commit | c1c7c36 |
| Classification | CONDITIONAL for freeze blanket |

## 2. Decision question

Should **ADR-008** be accepted as the governing Decision Log decision for **API Style / Versioning**?

If accepting the IS-101 recommendation: Versioned REST JSON /api/v1

## 3. Background

- Why open: IS-101 recommendation recorded; Decision Log acceptance never completed.
- Why it matters: API contract shape
...
1. Decision Log entry accepting ADR-008 with selected option.
...
- IS-101 ADR-008; `docs/adr/_index.md`
...
- G-10 effect: CONDITIONAL for freeze blanket | MG effect: API packages
