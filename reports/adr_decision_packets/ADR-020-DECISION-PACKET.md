...
| --- | --- |
| ADR ID | ADR-020 |
| Title | H-Drive Enforcement |
| Current status | OPEN / PROPOSED (not accepted) |
| Decision owner | Steve |
| Prepared by | Burt (Cursor) |
| Prepared against commit | c1c7c36 |
| Classification | BLOCKING_G10 tooling |

## 2. Decision question

Should **ADR-020** be accepted as the governing Decision Log decision for **H-Drive Enforcement**?

If accepting the IS-101 recommendation: Repository guard + cache redirects (IS-104)

## 3. Background

- Why open: IS-101 recommendation recorded; Decision Log acceptance never completed.
- Why it matters: ISSUE-HDRIVE-001; tooling before install
...
- Description: Accept IS-104 guard+redirect posture
- G-10: Disposition in Decision Log does not alone open G-10.
- Reversibility: SUPERSEDE via new ADR if needed.

### Option 2

- Description: Revise enforcement
- G-10: Disposition in Decision Log does not alone open G-10.
- Reversibility: SUPERSEDE via new ADR if needed.

### Option 3

- Description: Defer (not recommended)
- G-10: Disposition in Decision Log does not alone open G-10.
- Reversibility: SUPERSEDE via new ADR if needed.
...
- IS-101 ADR-020; `docs/adr/_index.md`
...
- G-10 effect: BLOCKING_G10 tooling | MG effect: Blocks package install / scaffolding
